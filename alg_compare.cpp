...
int main(int argc, char* argv[])
{
    const std::string file =
        (argc > 1) ? argv[1] : "graphs/USA-road-d.BAY.gr";
    const int  SRC        = 0;
    const double  DELTA      = 0.1;                 // same δ everywhere
    const int  THREADS    = 5;

    /* ---------- load graph ---------------------------------------- */
    Graph G;
    wideWeightRandomGraph(50, 200, 0, 10, G);
    G.print();
    ...
    */
    ...
    DeltaSteppingParallelDynamic  a_parD  (G, DELTA, THREADS, 60, 5);
   ...
   // checkEqual(a_ dij.tent, a_seqD.tent, "Dijkstra vs seq-dynamic");
    ...
    //checkEqual(a_dij.tent, a_parD.tent, "Dijkstra vs par-dynamic");
